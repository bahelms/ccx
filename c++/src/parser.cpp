...
void Parser::expect(std::string expected) {
  ...
  if (actual.value() != expected) {
    throw SyntaxError(std::format("Expected \"{}\" but got \"{}\"", expected,
                                  actual.value()));
  }
}
...
std::unique_ptr<Function> Parser::parse_function() {
  ...
  if (name.type() != TokenType::Identifier) {
    throw SyntaxError(std::format("Invalid function name: {}", name.value()));
  }
  expect("(");
  expect("void");
  expect(")");
...
std::unique_ptr<Exp> Parser::parse_exp() {
  ...
  if (token.type() == TokenType::Constant) {
    ...
    return std::make_unique<Unary>(std::make_unique<Complement>(), parse_exp());
  } else if (token.value() == "-") {
...
TEST_CASE("Parser::parse_exp for parenthesized expression") {
  ...
  parser = Parser(std::vector<Token>({{"~"},
                                      ...
                                      {")"},
                                      {")"}}));
  CHECK(parser.parse_exp()->to_string() == "Complement(Negate(Constant(100)))");
...
TEST_CASE("Parser::parse with extra tokens") {
  std::vector<Token> tokens{{"int"},    {"my_function", TokenType::Identifier},
                            {"("},      {"void"},
                            {")"},      {"{"},
